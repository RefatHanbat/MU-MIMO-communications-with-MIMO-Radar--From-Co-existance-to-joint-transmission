...
# ============================================================
# Figure07_reproduce_updated.py
#
# Updated from the user's previous Figure07_reproduce_corrected.py
#
# Fixes applied:
# 1) Sum-square penalty now correctly uses (alpha_i - Gamma*N0)^2
#    instead of alpha_i^2.
# 2) Figure 7 points are averaged over multiple channel realizations,
#    consistent with the axis labels "Average SINR" and "Average PSLR".
# 3) Weighted RCG methods are initialized from the constrained SDR
#    solution for the same realization and Gamma.
# 4) The weighted "max" branch is made more robust by trying two smooth
#    log-sum-exp temperatures and keeping the candidate with the lower
#    weighted objective. This reduces the pathological off-screen branch
#    seen in the previous code.
#
# Important note:
# This still cannot guarantee pixel-identical reproduction of Fig. 7,
# because the paper does not provide the exact random channel draws.
# ============================================================
...
def cost_grad_total(T, H, R2, Gamma_lin, N0, rho1, rho2, penalty_type, eps_lse=0.1):
    E = T @ herm(T) - R2
...
def rcg_total(T0, H, R2, P0, Gamma_lin, N0, rho1, rho2, penalty_type,
              max_iter=300, tol=1e-7, armijo_c=1e-4, armijo_beta=0.5,
              eps_lse=0.1, verbose=False):
    T = retract_total(T0, P0)
    f, G = cost_grad_total(T, H, R2, Gamma_lin, N0, rho1, rho2, penalty_type, eps_lse)
    ...
    for it in range(max_iter):
        ...
        for _ in range(30):
            ...
            fnew, Gnew = cost_grad_total(Tnew, H, R2, Gamma_lin, N0, rho1, rho2, penalty_type, eps_lse)
...
def cost_grad_per_ant(X, H, R2, Gamma_lin, N0, rho1, rho2, penalty_type, eps_lse=0.1):
    E = herm(X) @ X - R2
    fit = np.real(np.trace(E @ E))
    grad = 4.0 * rho1 * X @ E
...
def rcg_per_ant(X0, H, R2, P0, Gamma_lin, N0, rho1, rho2, penalty_type,
                max_iter=300, tol=1e-7, armijo_c=1e-4, armijo_beta=0.5,
                eps_lse=0.1, verbose=False):
    X = retract_per_ant(X0, P0)
    f, G = cost_grad_per_ant(X, H, R2, Gamma_lin, N0, rho1, rho2, penalty_type, eps_lse)
    ...
    for it in range(max_iter):
        ...
        for _ in range(30):
            ...
            fnew, Gnew = cost_grad_per_ant(Xnew, H, R2, Gamma_lin, N0, rho1, rho2, penalty_type, eps_lse)
            if fnew <= f + armijo_c * step * desc:
                accepted = True
                break
            step *= armijo_beta
...
def solve_max_robust_total(Tinit, H, R2, P0, Gamma_lin, N0, rho1, rho2, verbose=False):
    candidates = []
    for eps in [0.10, 0.05]:
        Tsol, fval = rcg_total(
            Tinit, H, R2, P0, Gamma_lin, N0, rho1, rho2, 'max',
            max_iter=350, tol=1e-7, eps_lse=eps, verbose=verbose
        )
        candidates.append((fval, Tsol))
    candidates.sort(key=lambda z: z[0])
    return candidates[0][1]


def solve_max_robust_per_ant(Xinit, H, R2, P0, Gamma_lin, N0, rho1, rho2, verbose=False):
    candidates = []
    for eps in [0.10, 0.05]:
        Xsol, fval = rcg_per_ant(
            Xinit, H, R2, P0, Gamma_lin, N0, rho1, rho2, 'max',
            max_iter=350, tol=1e-7, eps_lse=eps, verbose=verbose
        )
        candidates.append((fval, Xsol))
    candidates.sort(key=lambda z: z[0])
    return candidates[0][1]


# ------------------------------------------------------------
# Figure 7 pipeline with averaging across realizations
# ------------------------------------------------------------
def trace_curves(n_mc=10,
                 seed=7,
    ...
    for gdb in gamma_grid_db:
        ...
        bucket = {k: [] for k in curves.keys()}

        for mc in range(n_mc):
            H = cn((N, K), rng)

            covs_t, _ = solve_shared_constrained(H, R2, P0, Gamma_lin, N0, 'total', solvers, verbose=False)
            covs_p, _ = solve_shared_constrained(H, R2, P0, Gamma_lin, N0, 'per-ant', solvers, verbose=False)
            ...
            bucket[('constrained', 'total')].append((x, y))

            Tcov = sum(covs_p)
            x = average_sinr_db_from_covs(covs_p, H, N0)
            y = compute_pslr_db(beampattern(Tcov, plot_grid), plot_grid, 0.0)
            bucket[('constrained', 'per-ant')].append((x, y))

            # initial points for weighted solvers
            ...
            T_ss_t, _ = rcg_total(
                T0_total, H, R2, P0, Gamma_lin, N0, rho1, rho2, 'sum-square',
            ...
            bucket[('sum-square', 'total')].append((x, y))

            # max total
            rho1, rho2 = weights[('max', 'total')]
            T_m_t = solve_max_robust_total(T_ss_t, H, R2, P0, Gamma_lin, N0, rho1, rho2, verbose=False)
            x = average_sinr_db_from_beams(T_m_t, H, N0)
            y = compute_pslr_db(beampattern(T_m_t @ herm(T_m_t), plot_grid), plot_grid, 0.0)
            bucket[('max', 'total')].append((x, y))

            # sum-square per-ant
            rho1, rho2 = weights[('sum-square', 'per-ant')]
            X_ss_p, _ = rcg_per_ant(
                X0_per, H, R2, P0, Gamma_lin, N0, rho1, rho2, 'sum-square',
                max_iter=300, tol=1e-7, eps_lse=0.1, verbose=False
            )
            T_ss_p = herm(X_ss_p)
            x = average_sinr_db_from_beams(T_ss_p, H, N0)
            y = compute_pslr_db(beampattern(T_ss_p @ herm(T_ss_p), plot_grid), plot_grid, 0.0)
            bucket[('sum-square', 'per-ant')].append((x, y))

            # max per-ant
            rho1, rho2 = weights[('max', 'per-ant')]
            X_m_p = solve_max_robust_per_ant(X_ss_p, H, R2, P0, Gamma_lin, N0, rho1, rho2, verbose=False)
            T_m_p = herm(X_m_p)
            x = average_sinr_db_from_beams(T_m_p, H, N0)
            y = compute_pslr_db(beampattern(T_m_p @ herm(T_m_p), plot_grid), plot_grid, 0.0)
            bucket[('max', 'per-ant')].append((x, y))

        for key in curves:
            arr = np.array(bucket[key], dtype=float)
            if arr.size > 0:
                curves[key].append(np.mean(arr, axis=0))
...
def plot_fig7(curves):
    ...
    plt.tight_layout()
    plt.savefig('Figure07_reproduced_updated.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def reproduce_fig7_updated(n_mc=10, seed=7, verbose=False):
    curves = trace_curves(n_mc=n_mc, seed=seed, verbose=verbose)
    plot_fig7(curves)
...
if __name__ == '__main__':
    reproduce_fig7_updated(n_mc=10, seed=7, verbose=False)
