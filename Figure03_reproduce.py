...
def steering_vector(N, theta_deg, d = 0.5):
    ...
    # print(f"exponential part : {np.exp(1j * 2 * np.pi * d * np.sin(theta) * n)[:, None].shape}")
...
def solve_radar_only_separated(NR, P_R, angle_grid_deg, Pd, f_list, solvers, verbose = False):
    ...
    if R1.value is None or alpha.value is None:
        ...
        raise RuntimeError(f"Separted radar only problem ended with status {prob.status} using {used}")
...
def reproduce_fig3(seed = 7, beam_halfwidth_deg = 3.0, angle_step_deg = 1.0, verbose = False):
    ...
    beam_centers = np.array([-60.0, -36.0, 0, 36.0, 60.0])

    angle_grid = np.arange(-90.0, 90.0 + angle_step_deg, angle_step_deg)

    # print(f"angle_grid : {angle_grid}")
    ...
    R1, alpha1, st1, sv1 = solve_radar_only_separated(NR = NR, P_R = PR, angle_grid_deg = angle_grid, Pd = Pd,
                                                      f_list = [F[:,i] for i in range(K)], solvers = solvers, verbose = verbose )
    

    print("[2/4] Solving shared radar-only covariance R2.....")

    R2, alpha2, st2, sv2 = solve_radar_only_shared(N = N, P0 = P0_W, angle_grid_deg = angle_grid, Pd = Pd, solvers = solvers, verbose = verbose)

    print(f" status = {st2}, solver = {sv2}")

    print("[3/4] Solving separated Radcom SDR(19).....")

    W_list, sigma_sep, st3, sv3 = solve_separated_radcom( NC = NC, K = K, G = G, R1 = R1, A1 = A1, A2 = A2, Gamma_lin = Gamma_lin,
                                                         P_C = PC, N0 = N0_W, solvers = solvers, verbose = verbose)
    ...
    ax1.plot(angle_grid, p_radcom_sep_plot, 'r-', linewidth=1.8, label='Radar-Only')
